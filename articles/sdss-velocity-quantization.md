...
本記事で紹介した「P値 ($10^{-89}$)」の計算過程において、重大な計算ミス（または統計的な不備）が発覚しました。
現在、検証コードを見直し、正しい数値を再計算しています。
結論部分の「量子化の確定」については、一度取り下げて再検証とさせていただきます。
誤った情報を発信してしまい、申し訳ありません。修正版は追って報告します。
:::
